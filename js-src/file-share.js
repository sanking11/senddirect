...
class FileShare {
    constructor() {
        ...
        this.connectionType = 'unknown'; // 'direct', 'relay', or 'unknown'
        this.MONTHLY_RELAY_LIMIT_GB = 200; // 200GB monthly limit for relay transfers

        // VPN fallback mechanism
        this.forceRelayMode = false; // When true, forces relay-only connections (for VPN compatibility)
        this.connectionAttempt = 0; // Track connection attempts for auto-retry
        this.cachedIceServers = null; // Cache ICE servers for retry
        this.connectionTimeout = null; // Timeout for connection attempts
        this.maxConnectionAttempts = 2; // Max attempts (1 normal + 1 relay-only)
    ...
    // ============================================
    // TURN RELAY DETECTION AND USAGE TRACKING
    // ============================================

    // Detect if connection is using TURN relay or direct P2P
    async detectConnectionType() {
        ...
        try {
            ...
            console.log(`Connection types - Local: ${localType}, Remote: ${remoteType}`);
            ...
            if (localType === 'relay' || remoteType === 'relay') {
                ...
                // srflx (server reflexive) = NAT traversal via STUN, still P2P
                this.isUsingRelay = false;
                this.connectionType = 'direct';
                return 'direct';
            }
        } catch (err) {
            console.error('Error detecting connection type:', err);
            return 'unknown';
        }
    }

    // Get relay usage from localStorage (monthly tracking)
    getRelayUsage() {
        const currentMonth = new Date().toISOString().slice(0, 7); // YYYY-MM
        const stored = localStorage.getItem('relayUsage');
    ...
    // Update relay usage in localStorage
    updateRelayUsage(bytesTransferred) {
        const currentMonth = new Date().toISOString().slice(0, 7);
        const currentUsage = this.getRelayUsage();
        const newUsage = currentUsage + bytesTransferred;

        localStorage.setItem('relayUsage', JSON.stringify({
            month: currentMonth,
            bytesUsed: newUsage
        }));

        // Also report to server for global tracking
        this.reportRelayUsageToServer(bytesTransferred);

        return newUsage;
    }

    // Report relay usage to server
    async reportRelayUsageToServer(bytes) {
        try {
            await fetch('/api/relay-usage', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ bytes })
            });
        } catch (err) {
            console.error('Failed to report relay usage:', err);
        }
    }

    // Check if user has exceeded relay limit
    hasExceededRelayLimit() {
        const usedBytes = this.getRelayUsage();
        const limitBytes = this.MONTHLY_RELAY_LIMIT_GB * 1024 * 1024 * 1024;
        return usedBytes >= limitBytes;
    }

    // Get remaining relay quota
    getRemainingRelayQuota() {
        const usedBytes = this.getRelayUsage();
        const limitBytes = this.MONTHLY_RELAY_LIMIT_GB * 1024 * 1024 * 1024;
    ...
    // Show relay warning banner
    showRelayWarning() {
        // Remove existing warning if any
        const existing = document.getElementById('relayWarningBanner');
        if (existing) existing.remove();

        const usedBytes = this.getRelayUsage();
        const remainingBytes = this.getRemainingRelayQuota();
        const usagePercent = Math.round((usedBytes / (this.MONTHLY_RELAY_LIMIT_GB * 1024 * 1024 * 1024)) * 100);

        const banner = document.createElement('div');
        banner.id = 'relayWarningBanner';
        ...
        banner.innerHTML = `
            <div class="relay-warning-content">
                ...
                <button class="relay-warning-close" onclick="this.parentElement.parentElement.remove()">✕</button>
            </div>
            <div class="relay-usage-bar">
                <div class="relay-usage-fill" style="width: ${Math.min(usagePercent, 100)}%"></div>
            </div>
        `;

        // Add styles if not already added
        this.addRelayWarningStyles();

        // Insert at top of transfer section or body
        const transferSection = document.getElementById('transferSection');
        if (transferSection) {
            transferSection.insertBefore(banner, transferSection.firstChild);
        } else {
    ...
    showDirectConnectionBadge() {
        ...
        badge.id = 'directConnectionBadge';
        badge.className = 'direct-connection-badge';
        badge.innerHTML = `
            <span class="direct-badge-icon">🔗</span>
            <span class="direct-badge-text">Direct P2P - No relay limits</span>
        `;
        ...
        if (connectionStatus) {
            connectionStatus.parentElement.appendChild(badge);
        }
    }

    // Add CSS styles for relay warning
    addRelayWarningStyles() {
        if (document.getElementById('relayWarningStyles')) return;
        ...
        styles.textContent = `
            ...
            .relay-warning-text strong {
                color: #fb923c;
                font-size: 0.95rem;
            }
            .relay-warning-text span {
            ...
            .relay-usage-fill {
                height: 100%;
                background: linear-gradient(90deg, #4ade80, #fb923c, #ef4444);
                border-radius: 2px;
            ...
            .relay-limit-exceeded .relay-warning-text strong {
                color: #ef4444;
            }
        `;
    ...
    // Check relay limit before transfer
    async checkRelayLimitBeforeTransfer() {
        if (!this.isUsingRelay) return true; // Direct P2P, no limit

        if (this.hasExceededRelayLimit()) {
            this.showNotification('Monthly relay limit (200GB) exceeded. Please try again next month or use same network for direct P2P.', 'error');
            this.showRelayLimitExceededBanner();
            return false;
        }

        // Check if transfer would exceed limit
        const totalFileSize = this.files.reduce((sum, f) => sum + f.size, 0);
        const remaining = this.getRemainingRelayQuota();

        if (totalFileSize > remaining) {
            const canTransfer = this.formatRelayUsage(remaining);
            this.showNotification(`Not enough relay quota. Remaining: ${canTransfer}. File size: ${this.formatFileSize(totalFileSize)}`, 'error');
            return false;
        }
    ...
    showRelayLimitExceededBanner() {
        const existing = document.getElementById('relayWarningBanner');
        if (existing) existing.remove();

        const banner = document.createElement('div');
        banner.id = 'relayWarningBanner';
        banner.className = 'relay-warning-banner relay-limit-exceeded';
        banner.innerHTML = `
            <div class="relay-warning-content">
                <div class="relay-warning-icon">🚫</div>
                <div class="relay-warning-text">
                    <strong>Monthly Relay Limit Reached</strong>
                    <span>You've used your 200GB monthly relay quota. Direct P2P transfers are still unlimited.</span>
                    <span class="relay-usage">Limit resets on the 1st of next month.</span>
                </div>
            </div>
        `;
    ...
    async handleMessage(msg) {
        ...
        switch (msg.type) {
            ...
            case 'peer-joined':
                // Only host creates the offer - receiver waits for it
                if (this.isHost) {
            ...
            case 'retry-relay':
                // Peer is requesting relay-only mode (VPN fallback)
                await this.handleRetryRelay();
                break;
            ...
            case 'room-closed':
                // Don't overwrite "Transfer Complete!" status if transfer was successful
                if (!this.transferComplete) {
    ...
    async joinRoom() {
        try {
            ...
            await this.connectToServer();
            // Initialize WebRTC BEFORE joining room to avoid race condition
            // where offer arrives before peerConnection is ready
            await this.initializeWebRTC();
    ...
    async initializeWebRTC(forceRelay = false) {
        ...
        // Track relay mode
        this.forceRelayMode = forceRelay;
        this.connectionAttempt++;
        console.log(`FileShare: Initializing WebRTC (attempt ${this.connectionAttempt}, forceRelay=${forceRelay})`);

        // Fetch TURN credentials from server (or use cached)
        ...
        // Use cached ICE servers if available (for retry), otherwise fetch fresh
        if (this.cachedIceServers) {
            iceServers = this.cachedIceServers;
            console.log('FileShare: Using cached ICE servers');
        } else {
            try {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    iceServers = data;
                    this.cachedIceServers = data; // Cache for retry
                    console.log('TURN credentials loaded:', iceServers.length, 'servers');
                    iceServers.forEach((server, i) => {
                        console.log(`FileShare: ICE Server ${i}:`, JSON.stringify(server));
                    });
                } else {
        ...
        // - 'all': Try all connection types (direct P2P, TURN relay) - preferred for performance
        // - 'relay': Force all traffic through TURN relay - works through VPNs that block UDP
        const iceTransportPolicy = forceRelay ? 'relay' : 'all';
        console.log(`FileShare: Using iceTransportPolicy: ${iceTransportPolicy}`);
        ...
        this.peerConnection.onconnectionstatechange = async () => {
            ...
            if (state === 'connecting') {
                ...
                // Set connection timeout - if still connecting after 15s, try relay mode
                // This helps with VPNs that silently drop UDP packets
                if (!this.forceRelayMode && this.connectionAttempt < this.maxConnectionAttempts) {
                    this.connectionTimeout = setTimeout(() => {
                        if (this.peerConnection && this.peerConnection.connectionState === 'connecting') {
                            console.log('FileShare: Connection timeout, retrying with relay-only mode...');
                            this.retryWithRelayMode();
                        }
                    }, 15000);
                }
                ...
                // Detect connection type (relay vs direct P2P)
                const connType = await this.detectConnectionType();
                console.log('Connection type detected:', connType);

                if (connType === 'relay') {
                    this.showNotification('Connected via relay server', 'info');
                    this.updateConnectionStatus('Connected (Relayed)');
                    this.showRelayWarning();
                } else {
                    this.showNotification('Direct P2P connected!', 'success');
                    this.updateConnectionStatus('Direct P2P Connected');
                    this.showDirectConnectionBadge();
                }
                ...
                if (this.transferComplete) {
                    ...
                    // Try relay-only mode if we haven't already
                    if (!this.forceRelayMode && this.connectionAttempt < this.maxConnectionAttempts) {
                        console.log('FileShare: Connection failed, retrying with relay-only mode...');
                        this.retryWithRelayMode();
                    } else {
        ...
        this.peerConnection.oniceconnectionstatechange = () => {
            ...
            if (state === 'checking') {
                ...
                // Try relay-only mode if we haven't already
                if (!this.forceRelayMode && this.connectionAttempt < this.maxConnectionAttempts) {
                    console.log('FileShare: ICE failed, retrying with relay-only mode...');
                    this.retryWithRelayMode();
                } else {
    ...
    // Retry connection with relay-only mode (VPN fallback)
    // This is called automatically when normal connection fails
    async retryWithRelayMode() {
        console.log('FileShare: Retrying connection with relay-only mode (VPN fallback)...');
        this.showNotification('Trying VPN-compatible mode...', 'info');
        this.updateTransferStatus('Retrying with relay mode...');
        this.updateConnectionStatus('Switching to relay mode...');
        ...
        // Reinitialize with relay-only mode
        await this.initializeWebRTC(true); // forceRelay = true

        // Signal peer to retry connection
        // Host creates new offer, receiver waits for new offer
        if (this.isHost) {
            // Send retry signal to peer so they reinitialize too
            this.send({ type: 'retry-relay', roomId: this.roomId });
            // Create new offer after short delay to allow peer to reinitialize
            setTimeout(() => {
                this.createAndSendOffer();
            }, 500);
        } else {
            // As receiver, send retry signal and wait for new offer
            this.send({ type: 'retry-relay', roomId: this.roomId });
        }
    }

    // Handle retry-relay signal from peer (VPN fallback)
    async handleRetryRelay() {
        console.log('FileShare: Received retry-relay signal, switching to relay mode...');
        this.showNotification('Peer requested VPN-compatible mode...', 'info');
        this.updateTransferStatus('Switching to relay mode...');
        ...
        // Receiver will wait for new offer from host
    }
    ...
    async handleDataChannelMessage(event) {
        if (typeof event.data === 'string') {
            ...
            if (msg.type === 'file-info') {
                ...
                // Track relay usage on receiver side too
                if (this.isUsingRelay && this.receivedTotalBytes > 0) {
                    const newUsage = this.updateRelayUsage(this.receivedTotalBytes);
                    console.log('Relay usage updated (receiver):', this.formatRelayUsage(newUsage));
                }
    ...
    async startFileTransfer() {
        ...
        // Check relay limit before starting transfer (if using relay)
        if (this.isUsingRelay) {
            const canProceed = await this.checkRelayLimitBeforeTransfer();
            if (!canProceed) {
                this.updateTransferStatus('Transfer blocked - relay limit exceeded');
                return;
            }
        }
        ...
        // Track relay usage if using relay
        if (this.isUsingRelay) {
            const newUsage = this.updateRelayUsage(totalBytes);
            console.log('Relay usage updated:', this.formatRelayUsage(newUsage));
        }
    ...
    showTransferComplete() {
        ...
        // Update radar loader to show completed checkmark
        const radarLoader = document.querySelector('.radar-loader');
        if (radarLoader) {
            radarLoader.innerHTML = '<div style="font-size: 48px; color: #4ade80; animation: none;">✓</div>';
        }

        // Update share status title and text
        const statusTitle = document.querySelector('.status-title');
        const statusText = document.querySelector('.status-text');
        if (statusTitle) statusTitle.textContent = 'Transfer Complete!';
        if (statusText) statusText.textContent = 'All files have been transferred successfully.';
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Custom Cursor - REMOVED (handled by background.js)
    // All cursor code has been moved to background.js to avoid conflicts
    /*
    ...
    */ // End of commented out cursor code
});
