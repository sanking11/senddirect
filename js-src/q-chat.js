...
class QChat {
    ...
    async connectToServer() {
        return new Promise((resolve, reject) => {
            ...
            this.ws.onmessage = (event) => {
                ...
                console.log('Q-Chat: Received message:', msg.type, msg);
    ...
    async handleSignalingMessage(msg) {
        switch (msg.type) {
            ...
            case 'peer-joined':
                console.log('Q-Chat: peer-joined received, isHost:', this.isHost);
                this.showSystemMessage('Peer connected! Setting up secure channel...');
                this.updateStatus('connecting', 'Establishing P2P...');
                try {
                    await this.initializeWebRTC();
                    console.log('Q-Chat: WebRTC initialized, peerConnection:', !!this.peerConnection);
                    if (this.isHost) {
                        console.log('Q-Chat: Creating and sending offer...');
                        await this.createAndSendOffer();
                        console.log('Q-Chat: Offer sent');
                    }
                } catch (error) {
                    console.error('Q-Chat: Error in peer-joined handler:', error);
                    this.showSystemMessage('Connection setup failed');
                    this.updateStatus('disconnected', 'Setup failed');
                }
                break;
    ...
    async initializeWebRTC(forceRelay = false) {
        // Track relay mode for VPN fallback
        this.forceRelayMode = forceRelay;
        this.connectionAttempt++;

        console.log(`Q-Chat: Initializing WebRTC (attempt ${this.connectionAttempt}, forceRelay: ${forceRelay})`);

        // Use cached ICE servers if available (for retry), otherwise fetch
        ...
        if (!iceServers) {
            ...
            try {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    // Normalize ICE servers for Safari compatibility (url vs urls)
                    iceServers = data.map(server => {
                        const normalized = { ...server };
                        // Safari sometimes needs 'url' instead of 'urls'
                        if (server.urls && !server.url) {
                            normalized.url = server.urls;
                        }
                        return normalized;
                    });
                    console.log('Q-Chat: Using TURN servers for NAT traversal, count:', iceServers.length);
                    // Debug: Log full ICE server config to diagnose connection issues
                    iceServers.forEach((server, i) => {
                        console.log(`Q-Chat: ICE Server ${i}:`, JSON.stringify(server));
                    });
                } else {
                    console.log('Q-Chat: Invalid TURN response, using STUN fallback');
                }
            } catch (err) {
                console.log('Q-Chat: TURN fetch failed, using STUN fallback', err);
            }
            // Cache for potential retry
            this.cachedIceServers = iceServers;
        } else {
            console.log('Q-Chat: Using cached ICE servers for retry');
        }

        const config = {
            iceServers,
            // Use relay-only mode when forceRelay is true (VPN compatibility)
            // 'relay' forces all traffic through TURN servers (TCP/TLS), bypassing UDP which VPNs may block
            iceTransportPolicy: forceRelay ? 'relay' : 'all'
        };

        if (forceRelay) {
            console.log('Q-Chat: Using relay-only mode for VPN compatibility');
        }
        ...
        this.peerConnection.onicecandidate = (e) => {
            if (e.candidate) {
                console.log('Q-Chat: Sending ICE candidate:', e.candidate.candidate.substring(0, 50));
                this.send({
                    type: 'ice-candidate',
                    roomId: this.roomId,
                ...
                console.log('Q-Chat: ICE gathering complete');
            }
        };

        this.peerConnection.oniceconnectionstatechange = () => {
            console.log('Q-Chat: ICE connection state:', this.peerConnection.iceConnectionState);
        };

        this.peerConnection.onicegatheringstatechange = () => {
            console.log('Q-Chat: ICE gathering state:', this.peerConnection.iceGatheringState);
        };
        ...
        this.peerConnection.onconnectionstatechange = () => {
            ...
            if (state === 'connecting') {
                // Start connection timeout for VPN detection
                // VPNs often silently drop UDP packets, causing connections to hang
                if (this.connectionTimeout) {
                    clearTimeout(this.connectionTimeout);
                }
                this.connectionTimeout = setTimeout(() => {
                    if (this.peerConnection && this.peerConnection.connectionState === 'connecting') {
                        console.log('Q-Chat: Connection timeout - may be behind VPN');
                        if (!this.forceRelayMode && this.connectionAttempt < this.maxConnectionAttempts) {
                            this.showSystemMessage('Connection slow, trying VPN-compatible mode...');
                ...
                if (this.forceRelayMode) {
                    this.showSystemMessage('🔒 Secure connection established via relay (VPN mode)');
                } else {
                ...
                // Try relay mode if this was a normal connection attempt that failed
                if (state === 'failed' && !this.forceRelayMode && this.connectionAttempt < this.maxConnectionAttempts) {
                    console.log('Q-Chat: Connection failed, retrying with relay mode');
                    this.showSystemMessage('Direct connection failed, trying VPN-compatible mode...');
                    this.retryWithRelayMode();
                } else {
    ...
    async handleOffer(msg) {
        try {
            // Only initialize WebRTC if not already done (avoids overwriting peer connection)
    ...
    async handleIceCandidate(msg) {
        try {
            if (msg.candidate && this.peerConnection) {
                console.log('Q-Chat: Received ICE candidate:', msg.candidate.candidate?.substring(0, 50));
     ...
     * This forces all traffic through TURN servers using TCP/TLS
     */
    async retryWithRelayMode() {
        console.log('Q-Chat: Retrying with relay-only mode for VPN compatibility');

        // Signal peer to also reinitialize with relay mode
        this.send({
            type: 'retry-relay',
            roomId: this.roomId
        });
    ...
    /**
     * Handle retry-relay signal from peer
     * Reinitialize WebRTC with relay-only mode
     */
    async handleRetryRelay() {
        console.log('Q-Chat: Received retry-relay signal from peer');
        this.showSystemMessage('Peer requested VPN-compatible mode...');
